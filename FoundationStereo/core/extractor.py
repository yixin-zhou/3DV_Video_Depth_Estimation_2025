...
class ContextNetDino(MultiBasicEncoder):
    ...
    继承自MultiBasicEncoder，扩展了对DINOv2特征的处理能力
    用于提取图像的多尺度上下文特征
    """
...
class VideoDepthAnythingFeature(nn.Module):
    """
    VideoDepthAnything特征提取器
    
    从视频序列中提取深度相关特征
    使用预训练的VideoDepthAnything模型进行特征提取
    支持时间维度的特征融合
    """
    model_configs = {
    ...
    def forward(self, x_seq):
        ...
        返回:
            edge_features: 多尺度EdgeNeXt特征 [x4, x8, x16, x32]
            vit_feat: VideoDepthAnything的ViT特征
        """
...
class Feature(nn.Module):
    ...
    整合VideoDepthAnything和EdgeNeXt网络，提取视频序列的综合特征
    为立体匹配提供多尺度特征表示
    """
    def __init__(self, args):
        ...
        # 替换为 VideoDepthAnythingFeature
        self.dino = VideoDepthAnythingFeature(encoder=self.args.vit_size, num_frames=self.args.num_frames)
        self.dino = freeze_model(self.dino)
        vit_feat_dim = VideoDepthAnythingFeature.model_configs[self.args.vit_size]['features']//2
    ...
    def forward(self, x_seq):
        ...
        返回:
            multi_scale_feats: 多尺度特征列表 [x4, x8, x16, x32]
            vit_feat: VideoDepthAnything的ViT特征
        """
        B, T, C, H, W = x_seq.shape
